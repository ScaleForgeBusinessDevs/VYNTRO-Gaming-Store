  ...
  persist(
    (set, get) => ({
      ...
      addItem: (product, quantity = 1, selectedVariant = null, selectedColor = null) => {
        // Guard: if a size variant is marked unavailable or has price <= 0, do not add
        if (selectedVariant && (selectedVariant.isAvailable === false || Number(selectedVariant.price) <= 0)) {
          return;
        }
